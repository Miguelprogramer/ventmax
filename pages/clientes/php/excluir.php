...
<body>
    ...
    if(isset($_GET["excluir"])) {
        ...
        $cpfcli = filter_var($_GET["excluir"], FILTER_VALIDATE_INT);

        if ($cpfcli === false || $cpfcli <= 0) {
            echo "<p style='color:red;'>Erro: ID inválido!</p>";
        } else {
            // Utiliza Prepared Statement para evitar SQL Injection
            $stmt = $mysqli->prepare("DELETE FROM tb_clientes WHERE cpfcli = ?");
            $stmt->bind_param("s", $cpfcli);
            $stmt->execute();
            ...
            if ($stmt->affected_rows > 0) {
                ...
                echo "<p style='color:blue;'>Nenhum cliente encontrado com esse CPF.</p>";
            }
        ...
        echo "<p style='color:red;'>Erro: Nenhum CPF informado.</p>";
    }
    ?>
    <br>
